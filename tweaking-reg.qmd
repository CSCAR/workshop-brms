...
values to describe the parameters of interest. @fig-post-peng1 shows some
...
```{r peng1 posteriors}
#| code-fold: true
#| code-summary: Posteriors of average weights
#| fig-align: center
#| fig-width: 3.9
#| fig-height: 3
...
above 400 in most models [@Vehtari_2021]. Our penguin model above has way more
ESS than this: the lowest Bulk ESS is 3794 (for the `Intercept`) and the lowest
Tail ESS is 2698 (for the standard deviation `sigma`).
...
so our results are reliable [@Vehtari_2021]. Our penguin model shows that
...
averages. Note that Wikipedia was talking about the full ranges of weights,
not just averages, so our priors are comparatively broad.
...
In the code above, `normal(0, 1)` means "Normal distribution with mean equal to
zero and standard deviation equal to 1". I would like to name these parameters
explicitly, but then the code would not run. The reason is that the code
*inside* `prior()` is not for R but for Stan, so we have to conform to Stan's
syntax. And since writing things like `mean = 0` inside distributions is
illegal in Stan, we cannot write theme here either.

The [Stan Functions Reference](https://mc-stan.org/docs/functions-reference/)
shows how to write all the parameters in any distribution it has; the Normal,
for example, is
[here](https://mc-stan.org/docs/functions-reference/unbounded_continuous_distributions.html#distribution-statement).
...
#| code-fold: true
#| code-summary: Plot bill length by sex and species
#| warning: false
#| fig-align: center
#| fig-width: 5.5
#| fig-height: 3.5
...
discard at the beginning. `iter` (short for iterations) controls the number of random draws *per chain*, which includes the warmup. `cores` controls the
number of mini-brains in our computer that R will take. Each chain runs in one
core, so we never need more cores than chains. In practice, ask for at most
one core less than those available in your computer; the extra core will run
everything else (e.g., internet browsers or Excel).
...
bill that is between 1.85 and 3.26 mm longer on average than the female
penguin.
